...
- Capitalisation never decides a flag: a word counts as spelled correctly if the dictionary accepts any capitalisation of it. German nouns are capitalised in the dictionary and lower-case throughout keyword columns, and this app does not check case.
- In the compounding languages (`de`, `nl`, `sv`, `da`, `nb`), a rejected word is accepted when it splits into at most three dictionary words, allowing the linking letters (`s`, `es`, `n`, `en`, `e`, `er`) and parts of three letters or more. Splitting is only ever tried on a word the dictionary has already rejected.
- A suggestion that only changes a word's case is never offered: it would be a no-op fix.
